...
type PullRequest = Awaited<
	ReturnType<ReturnType<typeof getOctokit>["rest"]["pulls"]["get"]>
>["data"];

type Reaction = NonNullable<
	Parameters<ReturnType<typeof getOctokit>["rest"]["reactions"]["createForIssue"]>[0]
>["content"];

interface ScriptProps {
	context: Context & {
		payload: IssueCommentCreatedEvent | IssueCommentEditedEvent;
	};
	core: Core;
	exec: Exec;
	github: Octokit;
...
const fastForward = async (
	execGit: (args: string[]) => Promise<number>,
	{ pr }: { pr: PullRequest }
) => {
	const {
	...
	const isDownstream =
		pr.base.repo.fork && pr.head.repo != null && pr.base.repo.clone_url !== pr.head.repo.clone_url;

	// prefix head branch name with `origin/` which ensures branch names
	// containing `/` work properly
	const headOriginRef = pr.head.ref.startsWith("origin/") ? pr.head.ref : `origin/${pr.head.ref}`;
	...
	if (isDownstream) {
		...
		await execGit(["pull", "--ff-only", pr.head.repo?.clone_url || "<unknown head>", pr.head.ref]);
	} else {
...
const formatSummary = (message: string, title = "expand for details") =>
	`<details>
...
const verifyCheckSuccess = async ({
	context,
	github,
	pr
}: {
	...
	console.log(`Verifying checks (current workflow run: ${context.runId} #${context.runNumber})`);
...
`.trim()
	...
	const foundChecks = branchRules.find(({ type }) => type === "required_status_checks");
	...
	const checkRuns = requiredChecksResults.flatMap(
		({ data: { check_runs: resultCheckRuns } }) => resultCheckRuns
	);
...
const getIsMergeAllowed = async ({ pr }: { pr: PullRequest }): Promise<boolean> => {
	if (pr.base.ref === releaseBranch) {
		// merging into `origin/main` is only allowed from `origin/canary`
		return pr.head.repo?.clone_url === pr.base.repo.clone_url && pr.head.ref === prereleaseBranch;
	}
...
const createAddComment =
	(github: Octokit, scope: { owner: string; repo: string }, pullNumber: number) => (body: string) =>
...
export const releaseViaComment = async ({
	context,
	core,
	exec,
	github
}: ScriptProps): Promise<void> => {
	console.log(
...
`);
	...
	try {
		const execGit = (args: string[]) => exec.exec("git", args, execOptions);
